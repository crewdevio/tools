...
import { STATUS_TEXT } from "https://deno.land/std@0.119.0/http/http_status.ts";
 ...
 */
export async function readTrailers(headers: Headers, r: BufReader) {
...
export async function writeChunkedBody(w: BufWriter, r: Deno.Reader) {
...
export async function writeResponse(w: Deno.Writer, r: Response) {
  const protoMajor = 1;
  const protoMinor = 1;
